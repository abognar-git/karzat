# karzat — the engineering log

<sub>This is the project's build log, kept in the order things happened: every decision, every
correction, every number — and the numbers here are still live, recomputed by
`scripts/check_readme.py` on every test run. The front door is [../README.md](../README.md).</sub>
...
`karzat/seating.py` lays every MP on their real seat: 198 of 198 match, 76 seats stay
...
- [x] All 272 cycle-43 vote details synced (254 calls); every computed verdict agrees with the recorded result
...
- [x] **Motions on the MP pages** — the record's `<inditvanyok>` is counts only, so `iromanyok.cgi` (the current cycle's 539 irományok, submitters as "Név (Frakció)") is resolved to people with the same resolver and listed under each current-cycle MP: 1,017 submissions, and for every one of the 201 people the list count equals the record's "önálló" count; a parser bug that dropped every submitter of a multi-submitter bill was found and fixed on the way
...
- [x] **Számok** — the cycle month by month (`szamok/`): votes, the qualified-majority share of decisions, participation, dissent and agreement index per faction, as small charts with the table and `havonta.csv` beneath. The charts were a picture rather than a way in, so they now open outward in three ways. A month strip above them names every month of the cycle and, on hover or arrow key, says what the House did in it — votes, sitting days, votes a day, how many of the decisions wanted a qualified majority — with a link that opens the cycle's vote list narrowed to that month; one strip for all five charts rather than a link on every point of every one of them, which would put several hundred tab stops between a keyboard and the page. The legend is a plain colour key without JavaScript and a filter with it: one click leaves a single faction lit across all five charts at once, because three of them draw ten overlapping lines and the question is almost always about one faction and never about the tangle — and only what the legend names dims, since the qualified-majority line is a series like any other but belongs to no faction, and the first version made it vanish along with the rest. The vote count is drawn twice, per month and per sitting day, because the same bar can be two different months: December 2023 and December 2025 are 181 and 182 votes, which is the same bar twice, and 60.3 against 30.3 votes a sitting day, which is a fortnight of long days against an ordinary month. Both charts are built into the page and a script hides one, so without a script the reader gets the two stacked rather than a switch that does nothing. **The denominator was wrong when this shipped, and it is the part worth writing down.** `monthly()` counted the dates on which a vote was taken and the page called them ülésnap: 106 for cycle 42 against the record's own 192, so April 2023 announced 21.0 votes a day where the House sat 5 days and divided 4.2 times a day — a five-fold overstatement, in the accessible name of every month link, under a caption promising to show when the agenda was busy. 24 of that cycle's 39 months overstated the pace by half again or more, and the cycle index of the same cycle had been printing 192 all along, so two pages of one cycle disagreed by 86 and nothing compared them. The record lists its own numbered sittings from 1998, and the page uses them now, counted by number rather than by date because the House numbers two sittings on one date separately — the rule `sitting_weeks()` already followed. Cycles 34 and 35 have no such list (the API publishes none before 1998), so their chart is labelled *szavazási naponként* and says so, rather than borrowing a word for a number it does not have. April 2023 carried 21 votes across 5 sitting days, a pace of 4.2. Hovering any chart now answers instantly: a snap-to-nearest readout built from the points' own `<title>` labels — the browser showed the same figures, but after a second and only on a four-pixel dot — respecting the legend filter, cached per hover and recollected on scroll, pointer-only because every figure it shows is already in the table and on the points for everyone else. What let it through was that the README's gate recomputed the figure the same wrong way, so the check agreed with the page and the agreement read as verification; the gate now reads `sitting_days` from the committed index, and a test fails if the Számok page and the cycle index ever disagree about how many days a cycle sat. **Three more of the same kind came out of the same review.** Every chart on the page had been announcing itself by the wrong name: `svg_series` unpacks each series into a variable called `name`, which is also the parameter holding the chart's own title, so the accessible name read "DK: Fidesz, DK, MSZP…" — and the helper meant to inject the title replaced a literal the function never emitted, so it had returned its input unchanged for as long as it existed. axe cannot catch that: it checks that an accessible name exists, not that it says anything. The panel heading promised *lépkedj a nyilakkal* with no `keydown` bound anywhere, so the arrows scrolled the page — a control that is dead *with* a script, which is worse than the no-JS case these rules are written against; the strip takes Left/Right/Home/End now, clamped at the ends because a cycle is a timeline and not a ring, and 39 tab stops become one. And cycle 34's three faction charts were drawn with no data at all — empty axes labelled 100%, an empty legend, a `havonta.csv` of one header row — which reads as "these factions were at zero" rather than "the record does not say", because the API's `nev_szerint` is empty before 1998. Those charts are one sentence now, and every cycle states how many of its votes were roll calls: cycle 35's seven dots come from one roll call in 8,144, and nothing had said so. Three additions close the round, each an answer to "can the reader check this without me". Every chart panel carries a **Miből számoltuk** block — a native `<details>`, closed to one line, no script — stating numerator, denominator and exclusions in words verified against `analytics.py`: that presence checks are votes but not decisions, that they sit in the participation denominator and in the dissent denominator while nobody can dissent on one, that a tie means no faction position and so no dissenter, and that the agreement index is (largest camp − half the rest) / cast, presence checks excluded, cast-weighted by month. The table now holds every number the six charts draw — the sitting-day column with its per-day rate, the qualified share beside its count, the agreement index in each faction cell — because six accessible names promise *a számok a táblázatban* and for two charts that was false. And under the month strip one line names the cycle's maxima — most votes, densest month, most qualified-majority decisions — as real links into the same `?m=` drill-down, ties all named, because a record month should not depend on a reader walking 47 months with a mouse. The landing page also gains its twelfth door, **Beszédidő** — the floor-time pages were reachable only through a cycle page, which for one of the site's strongest features was a filing decision, not a design
...
- [x] **Felszólalások** — the sitting days' speech lists (`felszolalasok.cgi`, one call per day; `python3 -m karzat sync-speeches --ckl N`; `scripts/derive_speeches.py`): who spoke on which agenda item, what kind, in what role, for how long; speakers resolved with the roll calls' resolver (ministers and the President who are not MPs stay named, unresolved). Substantive vs procedural is a named list of kinds (`normalise.SUBSTANTIVE_KINDS` — chairing, announcements and the chair's result lines are procedural, personal-involvement remarks too), and the MP record's own per-cycle count agrees with it for 200 of 209 cycle-43 MPs and 218 of 220 in cycle 42 (the pages print both where they differ). Cycle 43: 5,053 rows over 25 sitting days, 2,129 substantive; cycle 42: 36,722 rows over 192 days. SQLite v3: `speech` and `mp_committee` tables. Also found: the API's `ulesnap` dates for the older cycles are one day early against their own weekday names — corrected from the weekday, flagged per row
...
- [x] **Felszólalások szövege és keresés** — `karzat sync-speech-texts --ckl N` fetches the record's text of every substantive speech (`felszolalas.cgi`, one call per speech; the payload names the MP by id where the speaker is one — it agrees with the lists' name resolution in every case), `scripts/derive_speech_texts.py` keeps them as plain paragraphs (the current cycle's file is committed; an earlier cycle's is tens of megabytes and stays local — fetch and derive it, the builder picks it up). Every substantive speech gets a page and a JSON twin (`felszolalas/<ülésnap>-<sorszám>`), every sitting day a page in the day's order grouped by agenda item; `felszolalas/kereses.html` searches the texts by word prefix (3+ letters, accents folded, terms AND-ed) over an index sharded by the token's first two letters, showing the record's own snippet; `feed/felszolalasok.xml` carries the recent speeches with their full text so a reader's filter can watch a keyword — that is the whole "topic radar": no topic classification, no summary, no server; committee sittings are not in the API and the page says so. Cycle 43: 2,129 texts, 8,005,865 characters. SQLite v4: `speech_text`
- [x] **Beszédidő** — mire ment el a Ház ideje, and not one label of it is mine. Every substantive speech carries an agenda item and four in five carry a bill number, both written by the Országgyűlés, so the page adds up seconds and groups them by the record's own strings: no topic model, no categories, nothing classified. The fragmentation that makes a year unreadable turns out to live entirely in the stage prefix — every agenda item naming a bill is written `<szakasz> <irományszám> <cím>`, and of the 179,714 that do, **0** fail to partition on the number — so dropping the stage from a debate's identity collapses "általános vita megkezdése / folytatása / lezárása" into one row and keeps the stages as their own breakdown. `analytics.floor_time` beside `cohesion` rather than a derive script: it is a groupby, not a fit. Three decisions that could each have been made dishonestly are named on the page — a joint debate is **one** row wearing every number it carried (splitting it invents a figure, charging it to the first understates the rest), the agenda slots that name no bill keep their own lines (napirend előtti alone is 15.8% of 2023 and is a time-box, not a subject), and faction size is the roll-call roster rather than a mandate count the W-API does not publish. Per cycle `beszedido/index.html` plus a page per year, the twenty longest debates of each expandable into faction × form × stage, `vitak.csv` / `bontas.csv` / `frakciok.csv` beside them. 8 cycles carry it — 201,454 substantive speeches, 13,529 hours, 1998 to 2026. What it shows that neither half shows alone: in 2023 the governing parties took 45.2% of the floor time on 67.8% of the roster, 0.67×. What the panel will not do is let that read as a silencing, because the alternative explanation is measurable and I measured it: across 199 faction-years since 1998 the ratio falls with size — factions under a twentieth of the House average 1.98× and those over half average 0.55× — because the vezérszónoki slot, allocated per faction, not per member — is 26.9% of that year and belongs to a faction however many members it has. Compare the largest faction of each era at its own size and a residual survives but nothing like the whole gap: before 2010 it ran 0.79× at 44.3% of the House, from 2010 on 0.55× at 58.3%, where a purely per-faction rule alone would predict 0.60×. So the panel says to compare like sizes, or one faction over time, rather than reading 1,00 as a floor. Tested rather than assumed, because it is the one thing that could invalidate the feature rather than a row of it: `duration_s` is the API's `videoido`, a video timecode, so if it carried the walk to the podium or clip padding every figure would be wrong in the same direction and every internal sum would still tie. The transcript is the outside witness — over the 17,613 speeches carrying both, characters against seconds correlate at 0.98 and the median rate is 910 characters a minute, ordinary Hungarian speech; padding would make short speeches yield fewer characters per second than long ones and the record does the reverse. Also flagged rather than smoothed: 3 sitting days whose own durations add up to more than a calendar day, which is not repeated rows and not a sitting past midnight, and the record does not say what it is. **Szónokok** — the same seconds person by person, because that is the question a reader actually arrives with: every speaker with measured substantive time, ranked by it, with the largest kind beside the total and the full per-kind split in `szonokok.csv`. It sits on `speaking_time()`, so the count-each-speech-once rule is inherited rather than reimplemented, and a test holds that the kind breakdown adds up to each speaker's own total. The panel's licence to rank is the caveat printed under it: a sum is a count, but two members' time is only comparable within a role, because the vezérszónoki slot belongs to the faction whoever delivers it — ministers and spokespersons appear too, with an empty faction cell, and nothing in the table calls anyone talkative. The reader ordered the page — Szónokok, then the kinds, then the stages — and both tables filter: the speakers by faction button and by typed name through the same machinery every other table uses, and the kind table by faction as one finished, builder-rendered table per faction that a script merely reveals, so no arithmetic ever runs in the browser and a variant's share column says it is the share of that faction's own time. The page's lede also lost its first person: the no-labelling claim reads the same in plain declarative — *a lap nem címkéz, csak összead*. And the record's kind labels got a translator: every jogcím on the page carries a plain-word definition — hover shows it instantly, and the same text stands in a glossary above the table for keyboard, touch and no-script readers. A test fails when a cycle brings a kind the glossary lacks, and the glossary's first line draws the editorial boundary: the label is the record's, the explanation is the page's. **Két olvasói hibajelzés zárta a kört, és mindkettő rendszerhibát talált.** A profil-mondat 56 képviselőnél az intézményt írta végzettségként — a rekord az iskolák közé sorolja a szakvizsgát és a tanfolyamot is, így állhatott ott minisztérium egyetem helyett; a sor most a képzettséget ÉS az intézményt együtt írja, átsorolás nélkül, mert egy szint-szó heurisztika a külföldi egyetemi diplomát fokozná le. A Javadalmazás-panel pedig kontextust kapott: ugyanannak a hónapnak a Ház-mediánja és a helyezés (a kormányfő alap-tiszteletdíja így már mond valamit), a kormányzati tisztség határmondata — az adatlap csak az országgyűlési javadalmazást közli —, letölthető javadalmazas.csv, és a derive mostantól előzmény-fájlba gyűjti a lekérdezett hónapokat, mert a forrás felejt, az oldal nem. Egy mondat épp e panel építése közben bukott meg a saját tesztjén: „a különbségek jellemzően a tisztségekkel járnak” — két képviselő a legmagasabb sávban áll látható tisztség nélkül, tehát a kauzális fél törölve, csak a tény maradt: az adatlap bontást nem közöl. A vagyonnyilatkozatokból összvagyont becsülni vagy a javadalmazással szembeállítani a lap nem fog: a nyilatkozat a tételek többségéhez nem ír értéket, az árazás modell lenne, nem számolás — az elutasítás indoklása ide van írva, hogy döntés maradjon, ne feledés. A hónap az egész Házra egy lapon is kint van — cklNN/javadalmazas/: mindenki egy sorban, frakcióra és névre szűrhetően, tisztség-rovattal, amely tájékoztat és nem magyaráz, tört-hónap jelzéssel ott, ahol a mandátum a hónap közben kezdődött vagy szűnt meg (a legkisebb összeg így magyarázza magát: a mandátum a hónap közepén járt le), és a profil-mondat iskola-választása is lexikai szűrőt kapott — a legutóbbi olyan sor, amelynek intézményneve maga mondja, hogy iskola; tövekre illesztve, mert a magyar többes szám a szóvégi a-t á-ra váltja, és mérve szállítás előtt: 2 521 sor marad, 127 lap javul, 20 esik vissza a teljes kiírásra. A javadalmazás-lap alá odaállt a törvény is: az Ogytv. tisztségi szorzói §-hivatkozással, két független kutató és egy döntnök által az njt.hu szövegéből ellenőrizve — a szorzók kizárólagosak, és a 106. § (2) a kulcs, amiért egy miniszterelnök az alapösszeggel áll itt: a kormánytag a kormányzati illetménye mellett kapja a képviselői tiszteletdíjat. A lap ezután megkereste azt az összeget, amelynek törvényi szorzói a sorokat forintra pontosan kiadják, és its statutory multiples land on 188 of the month's 199 stated amounts to the forint — a szorzó csak pontos egyezésnél jelenik meg, a többi sor kötőjel, mert az adatlap nem mond okot és a lap nem talál ki. **And then the pattern went site-wide as ordered: nine packages.** Eight definition families now live in the builder with one owner each — the speech kinds, the seven roll-call positions, the majority rules and results and voting modes, the chair's announcements, the mandate kinds, the gateway nouns, the iromány-number letters and the bill-record vocabulary — 73 definitions, every family drafted against the code and adversarially verified before a word shipped, and completeness tests per family so a rule majority.py can classify or a mode the indexes carry cannot exist without its sentence. The letters and the bill types are generated from the register's own `main_type`; the rest is typed once and wired everywhere by `def_term`, never retyped. The verification earned its keep twice on the way in: a drafted definition claimed the two-thirds base as "199 mandátum mellett 133 igen", which is one cycle's arithmetic printed on ten cycles' pages, and the sweep for machine language found the site's own earlier prose saying *gombot nyomott* about votes that include the oral név szerinti years — twelve sentences now say *szavazott*, because the record covers voting, not buttons
- [x] **A landing page, and one address per vote for good** — the site root is no longer the current cycle's directory but a page of its own: the chamber as it sits today drawn on the Országház floor plan with **every seat the member who holds it** — all 198 mandate-holders placed where their own record puts them, and with them the **12 nationality spokespersons** (`szoszolok.cgi` — a service I had registered and never called; they are elected by a nationality's list, sit in the back row of sectors 3–4, may speak and sit on committees, but cast no vote, so they are drawn as rings, kept out of every roster count and alignment, and their card says what they do instead). They also have a **career page** each under `szemely/` — the same address shape as an MP's, with committees and speech counts per cycle instead of votes, and a note that a cycle only appears there if the record's dated rows show it (the list itself is present-tense) — and their own kind in the search index (12 entries). The 59 seats still left as outlines are the room's spare places: it was built for 386 members, the House has had 199 since 2014 — hover or focus names them and shows the record their own page prints (leadott / frakciójával / ellene / érdemi felszólalás), a click pins the card, the name opens the page, the legend filters to one faction, and the arrow keys walk the seats; then the ten cycles since 1990 as stacked composition bars (mandates in force on each constituent sitting, read from the records' dated rows — 10 in the strip), the last sitting day with its votes and the freshness sentence, and the doors (Ki a képviselőm?, keresés, a szavak, a jelenlegi ciklus, pályaképek, módszer) as plain forms that work without JavaScript. Every cycle now lives under `ckl<N>/`, **the current one included** — before this, a vote's URL would have changed meaning at the next election, when the root switched to cycle 44; now `ckl43/szavazas/<slug>.html` is what it says it is, and the citations minted on those pages stay true
...
- [x] **Akadálymentességi és teljesítmény-kapu** — axe-core in a real browser over one page of every kind the builder can emit, both viewports, as a test. 160,396 HTML files is not something anyone audits, and nobody needs to: it is 41 templates with the record poured through them, and `tests/test_a11y.py` fails when a `build_*_page` appears that no page kind samples — the quiet way for a gate like this to stop working is for the site to grow past it. No dependency tree: axe-core 4.13 is vendored and SHA-256 pinned the same way DuckDB-WASM is, and node drives the system Chrome over the DevTools protocol using the WebSocket it has had built in since 22, so the whole harness is one 150-line script and no `node_modules`. A DOM shim would have been lighter and would have passed a page whose grey-on-grey nobody can read, because the contrast rule needs real layout. **What it found on its first run, none of it visible to someone who reads the site the way I do:** 34 scrollable regions no keyboard could reach, across twelve page kinds — every wide table scrolls sideways on a narrow screen and a container with `overflow-x:auto` and no `tabindex` can be dragged with a mouse and reached by nothing else; and a whole sitting day's vote navigation invisible, because the strip along the top of a vote page draws every vote as a link inside an `<svg role="img">`, and `role="img"` declares an element a leaf, so a screen reader announces one image and never reaches any of them. **Two of the defects were in the gate**, which is the part worth reporting: it measured pages mid-animation and reported four serious contrast violations per page that came and went with how many pages were in the batch (the boot script fades panels in from opacity 0 and correctly bails out under `prefers-reduced-motion`, which the audit now emulates), and it loaded from `file://`, where the pages' root-absolute asset paths point at the filesystem root — 200-odd phantom missing files that would have hidden a real one. A gate that fails on its own timing teaches people to switch it off. Performance comes from the same session rather than a second tool, because for a site with no framework and no third party a composite score would average the only four numbers that mean anything — bytes, requests, DOM nodes, render-blocking — into one that hides which of them moved; budgets are per kind and loose enough to catch a page that doubles rather than one that grew. Also gated now: `<html lang>`, a non-empty `<title>`, no sideways scroll at 390px, and that the fade-in actually finishes — a boot script that throws leaves every panel at opacity 0, and a blank page passes every rule there is
- [x] **A látogató nem megy sehova máshova** — the site's position has always been that a visit is known to nobody but this project's own log, and nothing checked it. 59 pages carried `<img src="https://www.parlament.hu/…">` for the 22 people with no local portrait — and 21 of those URLs answer 404, so a reader's browser called a third party in order to be told nothing and `onerror` removed the empty image before anyone noticed. `portrait_src` no longer falls back to the record's URL: there is our own copy or there is no picture, which is the honest version of not having one. A Commons file for a non-MP government member stays, licensed and credited on the page. The built site now makes 0 requests off its own origin, and a test reads every page to keep it that way
- [x] **Published** — the site is live on AWS: a private S3 bucket in Frankfurt behind CloudFront with an Origin Access Control, TLS on CloudFront's certificate, `index.html` as the default root object and the bucket's 403-on-missing rewritten to a real 404 page. `scripts/deploy_aws.py` does the whole thing from a named profile — creates the bucket and the distribution if they are absent, uploads only the objects whose size or MD5 differs, sets content types and cache headers per extension, and asks for an invalidation. First push: 132,387 objects, 3.57 GB, 7.2 minutes. Storage runs about ten cents a month and the traffic sits inside CloudFront's perpetual free tier. Directory addresses answer too: a CloudFront Function (`scripts/attach_rewrite.py`, attached once, not part of the upload path) rewrites a trailing-slash URI to its `index.html` and 301s the slashless form to the slash — a rewrite there would break every relative link on the served page, which is why it is a redirect
...
- [x] **A sitemap that is a recommendation, not an inventory** — `sitemap.xml` and a `Sitemap:` line in robots.txt, listing the doors and one career page per human the record has ever named. Not the vote pages: at the two-second crawl delay this site asks for, naming all of them would be a request to spend four days fetching what one Parquet file hands over in a single call, which is the argument robots.txt already makes — everything else is reachable by following links, which is what a crawler is for. The career pages are in it because a name is what somebody types into a search engine and `szemely/<azon>.html` is the durable answer: an MP's per-cycle page moves to a new address at every election and that one does not. It was written beside robots.txt at first, which is before the cross-cycle pages and the careers exist, so a build against an empty `site/` produced a sitemap of the ten cycles and nothing else, while a build over an existing tree found the previous run's `szemely/` on disk and looked correct. The failure is invisible in exactly the case anyone would test in. It is written last now, and the test counts the career pages in it against the ones on disk rather than checking that the word appears
...
- [x] **The pictures are ours, and they are small** — the pages had always linked a portrait where it lives rather than copying it, which is the polite thing to do and also the slow one: a 40 kB colour JPEG per face, fetched from another host, then turned grey by the browser on every paint. `scripts.pull_portraits` walks the 1,617 pictures the rosters name and caches the 1,596 that answer (22 return 404 and simply have no picture anywhere), and `scripts.make_portraits` renders each to a 192×256 grey WebP — twice the largest size any page draws it at, the tone baked in — averaging about 4 kB: 57.1 MB of colour JPEG becomes 6.3 MB. The build reads a committed manifest rather than the output directory, so the markup cannot drift from what was rendered; a person the manifest does not know keeps their original URL. CloudFront serves them for a month rather than a day. This is redistribution rather than a hotlink and parlament.hu states no terms of use for the pictures, so it stays what it was before: the owner's decision, recorded here
...
- [x] `sync-mps`: all 199 MP records; the **real seating plan** reconstructed from `<ulohely>` (`karzat/seating.py`, `scripts/derive_seating.py` → `data/derived/seating.json`) and drawn on the page — 196 of the hero vote's 199 placed, the 3 MPs whose mandates have since ended kept visible without a seat
...
```bash
python3 -m unittest discover -s tests -t .      # offline; 474 tests
...
resolver turns those into people; 1,017 submissions across the current cycle's 539
...
foreign-key check at the end; nothing is ever edited in place. Loaded today: 1,636 MPs (217 with a seat in their record — the 198
sitting, and one former MP whose record still carries a leftover seat — 1,602 with a Wikidata
QID, plus 1,438 rows for former MPs — every cached `kepviselo.cgi` record loads, and the
...
recorded result disagree — now across ten cycles and 49,771 roll calls, not 272 (each of the
...
Between 9 May and 28 August 2026 the Országgyűlés recorded 272 votes over 25 sitting
days: 271 decisions and 1 quorum check, 5 of them secret ballots. With every roll
call in hand (the secret ballots have none), the majority module's verdict agrees with the Országgyűlés's own
Elfogadva/Elutasítva on 272 of 272 — 0 disagreements — which is the first evidence
that the "present" interpretation is at least not contradicted by the record. 199 were plain
"Listás" votes; 48 needed two-thirds of those present, 7 two-thirds of all 199, 7
more than half of all 199, 5 four-fifths of those present — 69 qualified-majority
votes, a quarter of all decisions. 6 votes ordered exceptional procedure
(*kivételes eljárás*), 11 ordered urgent procedure, 5 approved a departure from the
House rules, and 59 accepted an interpellation answer. The chamber holds 198 members:
106 elected in single-member districts and 92 from the national list. T/71 — the
...
rules. Cycle 43 so far: 272 votes over 25 sitting days — 10.9 per sitting day — 69
qualified-majority votes, 25% of decisions; 6 exceptional-procedure orders and 11 urgent ones
...
vote. 54 records carry `képviselő elutasította a választ` in their log. The registry holds
586 full motion records for cycle 43 — the listing names 588 and the difference is one
number under which three different motions run, of which the service returns one — and 457 of
...
242 of the 437 oversight motions carry the exchange as it was spoken — 738
...
one is `Címzett`: filled on 437 of the 437 oversight instruments and on none of the
...
I do not badge the status. Every answered interpellation reads `elfogadva`, including all 54 where
...
in one piece — 79,842 votes, 17,446,580 cast positions,
3,263 member-cycles, and five more tables: the
495,305 speeches from the floor, the bills, the faction switches,
...
79,842 name more than one. A single column made that invisible — the value was
always present and always plausible. `szavazas_iromany` carries all 104,330 of
...
## Exit 2 in zero seconds

A reader asked why the site still said 28 August. The answer took an hour to find and was four faults deep,
and every one of them was a failure that had been reported and then discarded.

The nightly had been calling `python3 -m scripts.derive_speeches` with no `--cycle` since 18 August. The
script requires it, argparse printed a usage line and exited 2 in zero seconds, and the derive loop's
`allow_fail=True` moved on. Nothing looked wrong: one line among ninety in a log nobody reads when the run
ends green. Meanwhile `derive_speech_texts --cycle 43`, which does pass the flag, kept working. So the two
halves of the same record drifted apart inside our own pipeline — the verbatim **texts** for the 25th sitting
day were fetched and derived, while the **list** of that day's speeches stayed frozen on 18 August.

The symptom surfaced three weeks and one layer later. The echo index is built from the texts, so it found a
repeated passage in a speech from day 25 and linked to its page; the page is built from the list, so it was
never written. A broken internal link turned the gate red, the gate refused to publish, and the site stopped
moving. Every part of that is the system working correctly — a stale site is a smaller harm than a wrong one
— but the thing it was protecting against was a typo in an argv, and it took four days of red to say so.

**A step that cannot start is not a step that failed.** `allow_fail` exists so one bad derivation does not
take the chain down; it was never meant to cover a command that was written wrong. The loop now collects
which steps failed, names them, and refuses to build — because a site assembled from a fresh speech-text file
and a stale speech list is exactly the mixed-epoch output this project spends most of its effort preventing.

And the test that would have caught it on the first night costs nothing to run: read every argv the nightly
builds out of its source, read each target script's own `required=True` flags, and check that the one
satisfies the other. No subprocess, no data, no network. It found the bug immediately when the fault was
put back.

### The same shape, three more times

**The footer's facts froze on 20 August.** `derive_facts` is the only step that reads the SQLite corpus, and
nothing in the chain builds it. At first it exited 1 with a clear "no database" message, swallowed; then a
stray zero-byte `data/karzat.sqlite` — one `sqlite3 <path>` with no statement leaves one behind — made
`DB.exists()` true, and the clear message became `OperationalError: no such table: vote` from inside a query,
which reads like a schema bug rather than a missing input. Two weeks of a page stamping today's date over a
fact computed a fortnight ago. `karzat load` is now part of the chain, before the step that needs it, and the
script asks the file whether it holds the table rather than whether it exists. This is the derive-subset bug
for the third time: *an input a step needs is the chain's job to produce.*

**The nightly skipped thirty-six tests every night.** `node` lives under nvm; launchd's PATH is
`/usr/bin:/bin:/usr/sbin:/sbin` and nothing else, so `shutil.which("node")` answered None on a machine that
has node installed. The search tests, the share-link tests and the real JavaScript parse all skipped
themselves. The gate was materially weaker precisely when nobody was watching it. The lookup now goes where a
person would look — Homebrew, `/usr/local`, the nvm version directories — instead of trusting the environment
it was launched in.

**And the fallback for those skips reported a false failure.** With no node, the bundle test falls back to a
heuristic. Its first version counted apostrophes per line and failed on the correct bundle, because English
comments contain them. Its replacement looked for "a quote with nothing after it", which matches the
*closing* quote of every ordinary call: `classList.add('js');` reported itself as an unterminated string, and
the nightly went red for four days over a bundle that was fine. A heuristic that has cried wolf twice does
not get tuned a third time — it is a scanner now, walking the line and asking whether it ends inside a
string, and it is proved in both directions against real lines and deliberately broken ones.

The through-line is not that these were hard to find. It is that all four were *already being reported* —
exit 2, exit 1, thirty-six skip lines, a red test — into a log that only gets read when something else goes
wrong. The pipeline was loud and nobody was listening, which is the same thing as being silent.
