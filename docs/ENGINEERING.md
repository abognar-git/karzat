...
- [x] **Motions on the MP pages** — the record's `<inditvanyok>` is counts only, so `iromanyok.cgi` (the current cycle's 539 irományok, submitters as "Név (Frakció)") is resolved to people with the same resolver and listed under each current-cycle MP: 1,001 submissions, and for every one of the 201 people the list count equals the record's "önálló" count; a parser bug that dropped every submitter of a multi-submitter bill was found and fixed on the way
...
resolver turns those into people; 1,001 submissions across the current cycle's 539
...
564 full motion records for cycle 43 — the listing names 566 and the difference is one
number under which three different motions run, of which the service returns one — and 435 of
...
242 of the 423 oversight motions carry the exchange as it was spoken — 738
...
one is `Címzett`: filled on 423 of the 423 oversight instruments and on none of the
