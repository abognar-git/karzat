...
- [x] All 269 cycle-43 vote details synced (254 calls); every computed verdict agrees with the recorded result
...
- [x] **Motions on the MP pages** — the record's `<inditvanyok>` is counts only, so `iromanyok.cgi` (the current cycle's 539 irományok, submitters as "Név (Frakció)") is resolved to people with the same resolver and listed under each current-cycle MP: 999 submissions, and for every one of the 201 people the list count equals the record's "önálló" count; a parser bug that dropped every submitter of a multi-submitter bill was found and fixed on the way
...
```bash
python3 -m unittest discover -s tests -t .      # offline; 428 tests
...
resolver turns those into people; 999 submissions across the current cycle's 539
...
recorded result disagree — now across ten cycles and 49,771 roll calls, not 269 (each of the
...
Between 9 May and 27 August 2026 the Országgyűlés recorded 269 votes over 25 sitting
days: 268 decisions and 1 quorum check, 3 of them secret ballots. With every roll
call in hand (the secret ballots have none), the majority module's verdict agrees with the Országgyűlés's own
Elfogadva/Elutasítva on 269 of 269 — 0 disagreements — which is the first evidence
that the "present" interpretation is at least not contradicted by the record. 198 were plain
...
rules. Cycle 43 so far: 269 votes over 25 sitting days — 10.8 per sitting day — 69
qualified-majority votes, 26% of decisions; 6 exceptional-procedure orders and 11 urgent ones
...
562 full motion records for cycle 43 — the listing names 564 and the difference is one
number under which three different motions run, of which the service returns one — and 433 of
...
242 of the 421 oversight motions carry the exchange as it was spoken — 738
...
one is `Címzett`: filled on 421 of the 421 oversight instruments and on none of the
...
in one piece — 79,839 votes, 17,446,382 cast positions,
...
79,839 name more than one. A single column made that invisible — the value was
always present and always plausible. `szavazas_iromany` carries all 104,329 of
