...
```bash
python3 -m unittest discover -s tests -t .      # offline; 171 tests
...
site/              index.html — the first page, generated, guarded by --check and tests/test_site.py · assets/karzat.css, karzat.js — the shared look and the boot sequence, generated and checked the same way · per cycle (root = current, ckl<N>/ = earlier): szavazas/ (one page per vote + .json/.csv) · kepviselo/ (one per MP + .json/.csv, index) · iromany/ (one per bill, index) · kohezio/ · szoros/ · szamok/ · feed/ (Atom channels + their page) · adatok/ (tables + data dictionary) · site-wide: szemely/ (careers) · modszer/ · kereses/ (all generated, git-ignored)
