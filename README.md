...
- [x] **SQLite loader** (`karzat/load.py`, `python3 -m karzat load` / `stats`): schema v3 (v2 at the time) built from scratch from the cache in about twelve seconds for two cycles — 2,858 votes, 563,216 roll-call positions with 0 unresolved names, 3,580 faction-history rows, 3,278 mandates across cycles, 507 bills — plus a per-vote faction-plurality table and views (`v_vote`, `v_mp_alignment`) so discipline and absence are plain SQL
...
```bash
python3 -m unittest discover -s tests -t .      # offline; 179 tests
...
3,278 mandates back to 1990, 1,011 sitting days,
