...
- [x] **SQLite loader** (`karzat/load.py`, `python3 -m karzat load` / `stats`): schema v3 (v2 at the time) built from scratch from the cache — twelve seconds for two cycles then, twelve minutes for ten now — 62,544 votes, 14,025,106 roll-call positions with 0 unresolved names, 3,580 faction-history rows, 3,278 mandates across cycles, 507 bills — plus a per-vote faction-plurality table and views (`v_vote`, `v_mp_alignment`) so discipline and absence are plain SQL
...
```bash
python3 -m unittest discover -s tests -t .      # offline; 195 tests
...
resolver turns those into people; 974 submissions across the current cycle's 507
...
foreign-key check at the end; nothing is ever edited in place. Loaded today: 1,612 MPs (200 with a seat in their record — the 199
sitting, and one former MP whose record still carries a leftover seat — 1,602 with a Wikidata
QID, plus 1,413 rows for former MPs — every cached `kepviselo.cgi` record loads, and the
...
62,544 votes of which 40,896 carry a roll call — 14,025,106 positions, 0 unresolved names — 41,390
motions, 507 bills, and 251,217 rows of `vote_faction_majority` (each faction's plurality
...
consistency check that matters is in `stats`: 13 votes where the computed threshold and the
recorded result disagree — now across ten cycles and 40,896 roll calls, not 259 (each of the
13 is marked on its page and listed in the cycle's directory; the base corrections that took
