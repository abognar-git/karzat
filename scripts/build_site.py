...
def portrait_src(mp: dict) -> str:
    """Where the page should ask for this person's picture — our own copy, or nothing at all.

    A 192×256 grey WebP of about three kilobytes, against the forty the API serves in colour. The path is
    root-absolute because the same picture is drawn from four directory depths.

    This used to fall back to the record's own URL for anyone with no local copy, on the reasoning that a
    person added since the last render should still have a face. Two things were wrong with that. It made the
    reader's browser call parlament.hu — on 59 pages — from a site whose whole position is that a visit is
    known to nobody but this project's own log, and no page said so. And it did not even work: of the 22
    people it applied to, 21 of those URLs answer 404, so the browser made a third-party request to be told
    nothing, and `onerror` quietly removed the image. The honest version of "we have no picture of this
    person" is no picture.

    A Commons file for a non-MP government member is a different case and stays: it is a deliberate,
    licensed inclusion with its author printed beside it."""
    azon = mp.get("p_azon")
    if azon and azon in PORTRAITS:
        return f"/assets/portre/{azon}.webp"
    url = mp.get("photo_url") or ""
    return url if "wikimedia.org" in url or "wikipedia.org" in url else ""


def portrait_html(mp: dict, cls: str = "", alt: bool = False) -> str:
    ...
    The alt text is empty by default, and that is the accessible choice rather than a lazy one. Everywhere this
    is used, the person's name is written beside the picture — so a name in the alt makes a screen reader say
    it twice, once as an image and once as the text, which is noise pretended to be help. `alt=True` is for
    the one case where the picture stands alone with no name next to it."""
    src = portrait_src(mp)
    if not src:
        return ""
    ...
    text = f'{esc(mp.get("name") or "")} arcképe' if alt else ""
    return (f'<img class="{klass}" src="{esc(src)}" alt="{text}" width="{w}" height="{h}" loading="lazy" decoding="async" '
...
def podium_svg(pres: dict[str, list[dict]] | None, facs: list[dict], k: float = 1.0, width: float = 44.0) -> str:
    ...
    def dot(m, cx, cy, role):
        ...
        return (f'<g class="seat pres" tabindex="-1" data-az="{esc(m["azon"])}" data-f="{esc(m.get("faction") or "")}" data-role="{esc(role)}" aria-label="{esc(m["name"])} — az Országgyűlés {esc(role)}e">'
...
def seat_svg_real(view: dict, facs: list[dict], plan: dict, align: dict | None = None, szoszolok: dict | None = None,
    ...
    svg = (f'<svg viewBox="{-W:.0f} -{R+8:.0f} {2*W:.0f} {R+30:.0f}" role="group" aria-label="Az ülésterem rekonstruált ülésrendje: {len(view["positions"])} képviselő, frakció és szavazat szerint">'
...
def seat_svg_fallback(view: dict, facs: list[dict], align: dict | None = None, pres: dict | None = None) -> str:
    ...
    return ('<svg viewBox="-200 -200 400 222" role="group" aria-label="Ülésrend-diagram: minden képviselő egy helyen, frakciónként és szavazat szerint">'
...
def sitemap(out_dir: Path) -> str:
    """The addresses worth finding, which is not the same as every address.

    160,395 pages are built and this lists a few thousand. A sitemap is a recommendation, not an inventory,
    and at the two-second crawl delay this site asks for, naming every vote page would be a request to spend
    four days fetching what `adatok/szavazatok.parquet` hands over in one — which is the same argument
    robots.txt already makes. So: the doors and the people.

    The doors are the landing page, the cross-cycle pages, and each cycle's index and tools. The people are
    the career pages under `szemely/`, one per human the record has ever named, because a name is what
    somebody types into a search engine and those pages are the durable answer — an MP's per-cycle page moves
    to a new address every election, and the career page does not.

    Everything else is reachable by following links from these, which is what a crawler is for.
    """
    def when(p: Path) -> str:
    ...
    for rel, pri in (("index.html", "1.0"), ("kereses/index.html", "0.7"), ("szemely/index.html", "0.7"),
        ...
        f = out_dir / rel
        if f.exists():
            urls.append((rel, when(f), pri))
    for c in available_cycles():
        d = cycle_dir(c)
        for leaf, pri in (("index.html", "0.9"), ("kepviselo/index.html", "0.7"), ("iromany/index.html", "0.6"),
                          ("kohezio/index.html", "0.5"), ("szamok/index.html", "0.5"), ("szoros/index.html", "0.5"),
                          ("felszolalas/index.html", "0.6"), ("beszedido/index.html", "0.6"),
                          ("bizottsag/index.html", "0.5"), ("adatok/index.html", "0.5"),
                          ("feed/index.html", "0.4"), ("kepviselom/index.html", "0.6"), ("javadalmazas/index.html", "0.5"),
                          ("visszhang/index.html", "0.4")):
            f = out_dir / d / leaf
            if f.exists():
                urls.append((f"{d}{leaf}", when(f), pri))
    sd = out_dir / "szemely"
    if sd.is_dir():
        for f in sorted(sd.glob("*.html")):
            if f.name != "index.html":
                urls.append((f"szemely/{f.name}", when(f), "0.5"))
    base = SITE_URL or ""
    body = "".join(f"<url><loc>{esc(base)}/{esc(u)}</loc><lastmod>{m}</lastmod>"
...
def verdict_block(view: dict, inp: dict) -> str:
    ...
    return (tally +
            ...
            + def_glossary(
                ...
                + [(rl, RESULT_DEFS.get(rl)) for rl in [
                      "Elfogadva" if view.get("passed") is True else
                      "Elutasítva" if view.get("passed") is False else (view.get("result_raw") or "")]],
...
def roll_call_table(view: dict, inp: dict) -> str:
    ...
    return (f'<section class="panel deep">{CORNERS}<h2><span data-kz-text>Név szerinti lista</span><span class="tag">{len(rows)} képviselő{dl}</span></h2>'
            ...
            '<div class="tablewrap" tabindex="0"><table id="roll" data-page-size="25" data-counter="rn"><thead><tr><th scope="col" class="sortable" data-key="name">Képviselő</th><th scope="col" class="sortable" data-key="f">Frakció</th>'
...
def directory_row(v: dict) -> str:
    ...
    return (f'<tr data-rule="{esc(rule)}" data-result="{esc(v["result_raw"])}" data-y="{esc(v["on_date"][:4])}" data-m="{esc(v["on_date"][:7])}">'
...
def build_index(inp: dict, hero_ts: str) -> str:
    ...
    counts_cards = [
        ...
        (fl["qualified_majority_votes"], "minősített többségű szavazás", f'{hu_num(fl["decisions"])} döntésből'),
    ...
    if inp["closed"]:
        result_rows = "".join(f'<tr><td>{esc(k)}</td><td class="num mono">{hu_num(v)}</td></tr>' for k, v in fl["by_result"].items())
        month_rows = sorted(fl["by_month"].items())
        top = max((v for _, v in month_rows), default=1)
        months = "".join(f'<div class="row"><span>{esc(m)}</span><span class="stack"><i style="width:{100*v/top:.1f}%;background:var(--dim)"></i></span><span class="mono" style="text-align:right">{hu_num(v)}</span></div>' for m, v in month_rows)
        second_panel = (f'<section class="panel">{CORNERS}<h2><span data-kz-text>Eredmények és a ciklus ritmusa</span><span class="tag">{hu_num(fl["decisions"])} döntés</span></h2>'
                        f'<div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Eredmény</th><th scope="col" class="num">db</th></tr></thead><tbody>{result_rows}</tbody></table></div>'
                        f'<div class="hero-meta" style="margin-top:10px"><span class="lbl">szavazás havonta</span></div><div class="fbars" tabindex="0" style="max-height:260px;overflow:auto;margin-top:4px">{months}</div></section>')
    else:
...
<section class="grid">
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th>Szavazási mód</th><th class="num">db</th></tr></thead><tbody>{mode_rows}</tbody></table></div>
  </section>
  {second_panel}
</section>
<section class="panel deep" id="dir">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table data-page-size="25" data-counter="n"><thead><tr><th scope="col">Időpont</th><th scope="col">Tárgy</th><th scope="col">Szükséges többség</th><th scope="col" class="num">igen – nem – tart.</th><th scope="col" class="num">különbség</th><th scope="col">Eredmény</th></tr></thead><tbody id="rows">{dir_rows}</tbody></table></div>
</section>
""" + page_tail(inp, 0)
...
def day_strip(inp: dict, ts: str) -> str:
    ...
    for i, v in enumerate(votes):
        ...
        if m is None:                                        # a quorum check: no threshold, no margin
            cur_attr = ' aria-current="page"' if "now" in cls.split() else ''
            bars.append(f'<a href="{esc(v["slug"])}.html" class="{cls} q"{cur_attr}><rect x="{x:.1f}" y="{h / 2 - 1:.1f}" width="{w - gap:.1f}" height="2"/><title>{esc(title)}</title></a>')
            continue
        frac = min(1.0, abs(m) / peak)
        bh = max(1.6, frac * (h / 2 - 2))
        y = (h / 2 - bh) if m >= 0 else h / 2
        state = "pass" if m >= 0 else "fail"
        # "you are here" was a CSS class and nothing more, on 79,829 pages: the bar for the vote you are
        # reading is a link back to the same page, and nothing in the accessibility tree said which one.
        cur_attr = ' aria-current="page"' if "now" in cls.split() else ''
        bars.append(f'<a href="{esc(v["slug"])}.html" class="{cls} {state}"{cur_attr}><rect x="{x:.1f}" y="{y:.1f}" width="{w - gap:.1f}" height="{bh:.1f}"/>'
    ...
    return (f'<section class="panel strip">{CORNERS}'
            ...
            f'<div class="stripwrap" tabindex="0"><svg viewBox="0 0 {width:.0f} {h:.0f}" preserveAspectRatio="none" role="group" '
...
def mp_axis_panel(inp: dict, azon: str) -> str:
  ...
  <figure class="chartbox" tabindex="0"><svg viewBox="0 0 {W} {H}" role="img"
...
<section class="grid">
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th>Ciklus</th><th>Frakció</th><th>Időszak</th></tr></thead><tbody>{fac_hist or '<tr><td colspan="3">—</td></tr>'}</tbody></table></div>
    <div class="tablewrap" tabindex="0" style="border:0;margin-top:10px"><table><thead><tr><th>Ciklus</th><th>Választókerület / lista</th><th>Választás</th><th>Mandátum</th></tr></thead><tbody>{elections or '<tr><td colspan="4">—</td></tr>'}</tbody></table></div>
  </section>
  <section class="panel">{CORNERS}
    <h2><span data-kz-text>Benyújtott indítványok</span></h2>
    <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th>Ciklus</th><th class="num">önálló</th><th class="num">nem önálló</th></tr></thead><tbody>{motions or '<tr><td colspan="3">—</td></tr>'}</tbody></table></div>
    {items_html}
...
<section class="grid">
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Bizottság</th><th scope="col">Tisztség</th><th scope="col">Időszak</th></tr></thead><tbody>{com_rows or '<tr><td colspan="3">—</td></tr>'}</tbody></table></div>
  </section>
  <section class="panel">{CORNERS}
...
<section class="panel deep">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table data-page-size="25"><thead><tr><th>Időpont</th><th>Tárgy</th><th>Szavazata</th><th>Frakció többsége</th><th><span class="vh">A szavazás lapja</span></th></tr></thead><tbody>{dev_rows or '<tr><td colspan="5">Nincs ilyen szavazás.</td></tr>'}</tbody></table></div>
</section>
...
def axis_section(inp: dict) -> str:
  ...
  lapja rajzolja meg.</div>
    ...
    # This sentence used to read "the two ends of the line are the governing parties and the opposition",
    # printed on seven of ten cycles and gated on nothing but how well the model fitted. It was the one
    # sentence on the site that interpreted rather than counted — the record holds no government/opposition
    # mapping, so the label was mine, and the methodology page two clicks away says the axis's meaning is not
    # given in advance.
    #
    # There is a comparison that is not mine to make up: the order the votes produced, against the order the
    # factions physically sit in. The seating comes from the House's own floor plan, and the axis comes from
    # the roll calls, and neither knows about the other. Where they agree, the page says so and the reader
    # draws the conclusion; where they do not, that is the more interesting fact and it says that instead.
    seat_order = [f["id"] for f in inp["facs"] if f["id"] in ip["factions"]]
    axis_order = [f for f, _ in facs]
    # Pairwise, not exact: with ten factions an identical ordering essentially never happens, and it would
    # not be the interesting question anyway. What is interesting is how much of the seating the votes
    # reproduce — so count the faction pairs the two orders put the same way round, in whichever direction
    # agrees better, since the axis has no inherent left and right.
    pos_s = {f: i for i, f in enumerate(seat_order)}
    pos_a = {f: i for i, f in enumerate(axis_order)}
    pairs = [(a, b) for i, a in enumerate(seat_order) for b in seat_order[i + 1:] if b in pos_a and a in pos_a]
    agree = sum(1 for a, b in pairs if (pos_s[a] < pos_s[b]) == (pos_a[a] < pos_a[b]))
    agree = max(agree, len(pairs) - agree)
    strong = not (ip["apre"] < 0.75 or ip["second_dimension"] > 0.02)
    # The seating comparison is an independent witness on exactly one cycle: the current one, where every
    # seat comes from the record's own ulohely. For the archive cycles inp["facs"] is config/factions.yml —
    # this project's hand-assigned order — and the QA round caught the page comparing the votes against our
    # own configuration while calling the two independent. There the sentence stays silent about seating.
    seat_is_record = not inp["closed"]
    found = (f'A sorrendet senki nem adta meg előre — a szavazatokból jött ki. És nagyrészt <b>ugyanaz a '
             f'sorrend, ahogy a frakciók az ülésteremben ülnek</b>: {hu_num(len(pairs))} frakciópárból '
             f'{hu_num(agree)} áll ugyanúgy. A Ház ülésrendje és a szavazatokból becsült vonal egymásról nem '
             f'tud semmit.'
             if seat_is_record and strong and pairs and agree / len(pairs) >= 0.8 else
             f'A sorrendet senki nem adta meg előre — a szavazatokból jött ki, és <b>nem követi</b> az '
             f'ülésrendet: {hu_num(len(pairs))} frakciópárból csak {hu_num(agree)} áll ugyanúgy.'
             if seat_is_record and strong and pairs else
             'A sorrendet senki nem adta meg előre — a szavazatokból jött ki.')
  ...
  {warn}
  <figure class="chartbox" tabindex="0" style="margin-top:12px"><svg viewBox="0 0 {W} {H}" role="img"
...
def build_cohesion_page(inp: dict, co: dict) -> str:
    ...
    for f in facs:
        ...
        pair_blocks.append(f'<h3 class="mono" style="font-size:10px;letter-spacing:.2em;color:var(--dim2);margin:12px 0 4px">{esc(f)} — a legkevésbé egyező párok</h3>'
                           f'<div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Pár</th><th scope="col" class="num">Egyező / közös</th><th scope="col" class="num">Egyezés</th></tr></thead><tbody>{prs}</tbody></table></div>')
...
def coverage(v: dict) -> str:
    """Which cycles a table actually holds.

    Three of the eight do not reach 1990 — the bills are the current cycle only, the committee
    seats are today's — and a listing that gave sizes but no span let all eight read as though they
    covered the whole corpus. The reader should not have to run a GROUP BY to find the edge."""
    c = v.get("cycles")
    if not c:
        return "jelenlegi állapot"
    return f"{c[0]}. ciklus" if c[0] == c[1] else f"{c[0]}–{c[1]}. ciklus"
...
def build_report_page(inp: dict) -> str:
    ...
    pq = {n: v for n, v in ((inp.get("parquet") or {}).get("tables") or {}).items()
          if (SITE_DIR / "adatok" / f"{n}.parquet").exists()}   # the manifest is committed; the files are not
    have = (SITE_DIR / "assets" / "duckdb" / "duckdb-eh.wasm").exists()   # SITE is the index file, not the tree
    runtime_mb = (f"~{hu_dec(runtime_wire_bytes() / 1e6, 1)} MB" if have
                  else "nincs telepítve")     # a clone without it used to quote a size for a file it lacks
    ex = "".join(
    ...
    rows = "".join(
        ...
        f'<td class="sub mono">{esc(coverage(v))}</td>'
...
<section class="panel deep">{CORNERS}
  ...
  <div id="chartwrap" hidden style="margin-top:14px">
    ...
    <figure id="fig" class="chartbox" tabindex="0"></figure>
  </div>
...
<section class="panel">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table><thead><tr><th>Fájl</th><th>Egy sor</th><th>Meddig ér</th><th class="num">Sor</th><th class="num">Méret</th><th>Oszlopok</th></tr></thead><tbody>{rows}</tbody></table></div>
...
<noscript><div class="hero-meta prose" style="margin-top:14px">Ez a lap egy adatbázist futtat a
böngésződben, tehát JavaScript nélkül nem működik — a mező és a gombok itt vannak, de nem csinálnak semmit.
A teljes rekord így is letölthető: a <a href="../adatok/szavazatok.parquet">szavazatok</a>, a
<a href="../adatok/szavazasok.parquet">szavazások</a> és a <a href="../adatok/kepviselok.parquet">képviselők</a>
táblája Parquetben, és minden ciklus <span class="mono">adatok/</span> lapján CSV-ben is. Hogy mi van bennük:
<a href="../modszer/index.html">a módszer</a>.</div></noscript>
...
<section class="panel deep">{CORNERS}
  ...
  <a href="{"../" * (1 + inp["base_depth"])}modszer/index.html#visszhang">módszer</a>.</div>
</section>
...
def bill_path_html(inp: dict, rec: dict, votes: list | None = None) -> str:
    ...
    svg = (f'<svg viewBox="0 0 {W:.0f} {Y + 30:.0f}" role="group" aria-label="{esc(title_h)} {esc(hu_date(lo))} és {esc(hu_date(hi))} között: '
    ...
    return (f'<section class="panel deep road">{CORNERS}'
            ...
            + svg
            + (f'<div class="tablewrap" tabindex="0" style="margin-top:12px"><table><thead><tr><th>Tárgyaló bizottság</th><th>Jogcím</th><th>Házszabály</th></tr></thead><tbody>{coms}</tbody></table></div>' if coms else "")
            + f'<details class="evlog"><summary>Az adatlap teljes eseménynaplója ({hu_num(len(log))} sor)</summary>'
              f'<div class="tablewrap" tabindex="0"><table data-page-size="60"><thead><tr><th>Dátum</th><th>Esemény</th><th>Felszólalás</th></tr></thead><tbody>{"".join(log)}</tbody></table></div></details>'
            + '</section>')
...
def motion_card_html(inp: dict, rec: dict) -> str:
    ...
    body = "".join(f'<tr><th scope="row">{k}</th><td>{hu_num(v)}</td></tr>' for k, v in rows if v)
    ...
    return (f'<section class="panel deep">{CORNERS}<h2><span data-kz-text>{head}</span></h2>'
            f'<div class="tablewrap" tabindex="0"><table class="kv2"><tbody>{body}</tbody></table></div>{note}</section>')
...
def build_bill_page(inp: dict, b: dict) -> str:
    ...
    # `span` is the first and last ROLL CALL, and it used to be printed after the word "benyújtva" — so 108
    # pages said a motion was submitted on the day it was voted on, and 19 of them gave a date range after a
  ...
  <div class="tablewrap" tabindex="0"><table data-page-size="50"><thead><tr><th scope="col">Időpont</th><th scope="col">Szám</th><th scope="col">Szakasz</th><th scope="col">Szabály</th><th scope="col" class="num">igen – nem – tart.</th><th scope="col">Eredmény</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div>
...
# The Számok faction columns' three metrics, defined once: the header's hover, and the "Miből számoltuk"
# blocks' opening sentences, read from here — the two cannot drift because there is only one sentence.
METRIC_DEFS: dict[str, str] = {
    "részvétel": "Azt mutatja, hogy a frakció tagjai a szavazások mekkora részén adtak le szavazatot.",
    "ellene": "Azt mutatja, milyen gyakran szavazott valaki másképp, mint a saját frakciójának a többsége.",
    "index": "Azt méri, mennyire szavazott egyben a frakció.",
}


def term_as(text: str, definition: str) -> str:
    """A term span whose visible text differs from the definition's key — a header abbreviation, a plural."""
    return f'<span class="term" data-def="{esc(definition)}">{esc(text)}</span>'
...
def build_numbers_page(inp: dict, ms: list[dict]) -> str:
    ...
    fac_charts = [
        ...
        ("Egyetértési index frakciónként", svg_series(labels, ser("ai"), "lines", 1.0, lambda v: hu_dec(v), name="Egyetértési index frakciónként"), "leadott szavazatokkal súlyozott havi átlag",
         METRIC_DEFS["index"] + ' 1,00 = mindenki, aki szavazott, ugyanazt '
    ...
    if n_roll:
        blocks += (f'<section class="panel">{CORNERS}<h2><span data-kz-text>Mire épülnek a frakciószámok</span></h2>'
                   ...
                   f'csak a végeredményt ismerjük. Ahol az ábrán nincs pont, ott az adott frakciónak abban a hónapban nincs '
                   f'név szerinti szavazata — mert nem volt ilyen szavazás, vagy mert a frakció akkor már nem '
                   f'működött. Az nem nulla, hanem hiányzó adat.</p></section>')
    else:
    ...
    # month, so a chart stops being a dead end — you can see that December 2023 was busy and then go and read
    # what the House was voting on. Without a script the links still work and simply land on the whole list.
    strip = "".join(
        # One level up, inside the cycle — szamok/ is a sibling of the cycle index. Writing the prefix as
...
"""
    ...
    sub3 = " · ".join(term_as(t, METRIC_DEFS[t]) for t in ("részvétel", "ellene", "index"))
    head = "".join(f'<th scope="col" class="num">{esc(f)}<span class="sub">{sub3}</span></th>' for f in facs)
    def cell(x):
...
def build_data_page(inp: dict, out_dir: Path | None = None) -> str:
    ...
    files = [("szavazasok.csv", "szavazasok.json", "minden szavazás, egy sor egy szavazás"),
             ...
             ] + ([("javadalmazas.csv", None, "képviselőnként a lekérdezés havának bruttó országgyűlési javadalmazása, ahogy az adatlap közli — kormányzati illetmény nincs benne")]
                  if not inp["closed"] and any(m.get("remuneration") for m in inp["mps"].values()) else []) + [
    ...
    dic = "".join(f'<h3 class="mono">{esc(title)}</h3><div class="tablewrap" tabindex="0" style="border:0"><table><tbody>' + "".join(f'<tr><td class="mono" style="white-space:nowrap">{esc(col)}</td><td>{esc(mean)}</td></tr>' for col, mean in cols) + '</tbody></table></div>'
                  for title, cols in ex.adatszotar() if has_floor(inp) or not title.startswith("beszedido/"))
...
<section class="panel">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Fájl</th><th scope="col">Tartalom</th></tr></thead><tbody>{rows}</tbody></table></div>
</section>
<section class="panel">{CORNERS}
...
<section class="panel deep">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table id="biz" data-page-size="50" data-counter="bn"><thead><tr><th scope="col">Bizottság</th><th scope="col">Elnök</th><th scope="col" class="num">Tag</th><th scope="col">Következő ülés</th></tr></thead><tbody>{"".join(rows) or '<tr><td colspan="4">—</td></tr>'}</tbody></table></div>
</section>
""" + page_tail(inp, 1)
...
def build_committee_page(inp: dict, key: str, e: dict) -> str:
    ...
    if api:
        ...
        now_panel = (f'<section class="panel">{CORNERS}<h2><span data-kz-text>Mai összetétel</span><span class="tag">az API bizottsági adatlapja szerint · {n_now} fő</span></h2>'
                     f'<div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Név</th><th scope="col">Frakció</th><th scope="col">Tisztség</th></tr></thead><tbody>{now_rows or "<tr><td colspan=3>—</td></tr>"}</tbody></table></div></section>')
    ...
    for sub in e.get("subs") or []:
        ...
        subs_html += (f'<section class="panel">{CORNERS}<h2><span data-kz-text>Albizottság</span><span class="tag">{esc(sub["name"])}</span></h2>'
                      f'<div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Név</th><th scope="col">Frakció</th><th scope="col">Tisztség</th></tr></thead><tbody>{srows or "<tr><td colspan=3>—</td></tr>"}</tbody></table></div></section>')
...
<section class="panel deep">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table data-page-size="50"><thead><tr><th scope="col">Név</th><th scope="col">Frakció</th><th scope="col">Tisztség</th><th scope="col">Időszak</th></tr></thead><tbody>{hist or '<tr><td colspan="4">—</td></tr>'}</tbody></table></div>
</section>
...
<section class="panel">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Kör</th><th scope="col">Mi van benne</th><th scope="col">Fájl</th></tr></thead><tbody>
...
<section class="panel">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Frakció</th><th scope="col" class="num">Különvélemény</th><th scope="col">Fájl</th></tr></thead><tbody>{fac_rows or '<tr><td colspan="3">—</td></tr>'}</tbody></table></div>
</section>
<section class="panel">{CORNERS}
...
<section class="panel deep">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table><thead><tr><th scope="col">Időpont</th><th scope="col">Tárgy</th><th scope="col" class="num">Különvélemény</th><th scope="col">Frakciók</th></tr></thead><tbody>{latest or '<tr><td colspan="4">Ebben a ciklusban nem volt.</td></tr>'}</tbody></table></div>
</section>
""" + page_tail(inp, 1)
...
def speaking_time(inp: dict) -> dict:
    ...
    chair's result lines. Roughly half the rows and a tenth of the seconds, cycle by cycle: the chair speaks
    often and briefly. The exact shares are counted here (`technical_rows`, total − substantive) and the page
    prints those — a typed figure stood in this docstring and on eight pages for a month, and it was wrong."""
    if "_speaking" in inp:
        return inp["_speaking"]
    per_mp: dict[str, dict] = {}
    per_fac: dict[str, dict] = {}
    total = subtotal = counted = reprints = tech_rows = 0
    ...
    inp["_speaking"] = {"per_mp": per_mp, "per_faction": per_fac, "total": total, "substantive": subtotal,
                        "rows_counted": counted, "reprints_skipped": reprints, "technical_rows": tech_rows}
    return inp["_speaking"]
...
def speaking_panel(inp: dict) -> str:
    ...
    return (f'<section class="panel">{CORNERS}'
            ...
            f'eredményét. Sok ilyen sor van — a soroknak {hu_dec(100 * sp["technical_rows"] / (sp["rows_counted"] or 1), 0)} százaléka —, '
            f'de az időnek csak {hu_dec(100 * (sp["total"] - sp["substantive"]) / (sp["total"] or 1), 0)} százalékát adja: az elnök sokszor '
...
def build_speech_page(inp: dict, r: dict, text: dict | None, prev_r: dict | None, next_r: dict | None, bs: dict | None) -> str:
    ...
    # 23,919 speech pages shared a title with at least one other, nineteen of them on one string: the same
    # member, the same day, the same kind of speech. In a search result or a row of browser tabs those pages
    # are indistinguishable, and the thing that tells them apart — what was being debated — was already in
    # the description one line below. The bill number where the item names one, the agenda item otherwise,
    # and the sequence number as the last resort, because two speeches can share even that.
    # The bill number is what the speech was about; the sequence number is which speech it was. Both are
    # needed: a long committee debate can have the same member speaking thirteen times on the same bill on
    # the same day, and "Jámbor András Imre · 2022-10-26 · T/1620 · felszólalás" was thirteen pages with one
    # name between them. The sequence is the record's own numbering of the day and is already in the address.
    subject = (r.get("iromany") or "").split(" · ")[0].strip() or cut(r.get("event") or "", 40)
    mark = (f' · {subject}' if subject else "") + (f' · {r["seq"]}.' if r.get("seq") is not None else "")
    return page_head(f'{who} · {r["date"]}{mark} · {cut(r.get("kind") or "felszólalás", 40)}{" · " + str(inp["cycle"]) + ". ciklus" if inp["closed"] else ""} · karzat',
...
def build_day_page(inp: dict, ulnap: int, day_rows: list[dict], texts: dict) -> str:
    ...
    for r in sorted(day_rows, key=lambda r: r["order"]):
        ...
        groups[-1][1].append(r)
    trs = []
    # One speech listed under several agenda items is printed under each of them, and each printing carried
    # the same row id — `s1-38` twenty-one times on one day. A duplicate id makes `#s1-38` land wherever the
    # browser feels like, which is the anchor every link to a speech on this page uses. The first printing is
    # the canonical one and keeps the id; the rest are the same speech seen again and are marked `reprint`.
    seen_ids: set[str] = set()
    for ev, rows in groups:
        trs.append(f'<tr class="grp"><td colspan="5"><b>{esc(ev)}</b></td></tr>')
        for r in rows:
            sid = speech_id(r)
            anchor = f' id="s{esc(sid)}"' if sid not in seen_ids else ' data-reprint="1"'
            seen_ids.add(sid)
...
<section class="panel deep">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table data-page-size="20"><thead><tr><th scope="col">Nap</th><th scope="col">Felszólaló</th><th scope="col">Napirendi pont · részlet</th></tr></thead><tbody id="spres"><tr><td colspan="3" class="hero-meta">A kereső JavaScriptet használ, és a böngésződben nem fut. Nélküle: a napok listája az <a href="index.html">előző oldalon</a>, és minden felszólalás szövege a saját lapján — a teljes lista <span class="mono">../adatok/felszolalasok.csv</span>.</td></tr></tbody></table></div>
...
<section class="panel">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table data-page-size="25" data-counter="dn"><thead><tr><th scope="col">Nap</th><th scope="col" class="num">Ülésnap</th><th scope="col" class="num">Sor</th><th scope="col" class="num">Érdemi</th></tr></thead><tbody>{trs or '<tr><td colspan="4">—</td></tr>'}</tbody></table></div>
...
def long_day_note(inp: dict) -> str:
    ...
    return (f' <b>Egy figyelmeztetés a számokhoz:</b> {hu_num(len(bad))} olyan ülésnap van ebben a ciklusban, '
            ...
            f'{which}. '
            + ('Az eddig ismert három napra kézzel ellenőrizve: nem az okozza, hogy egy felszólalás kétszer '
               'szerepelne, nem is egyetlen kirívóan hosszú időadat, és nem éjfélen átnyúló ülés — a következő '
               'naptári napon külön ülésnap van. ' if len(bad) <= 3 else
               'A korábbi három ilyen nap kézi ellenőrzésen átesett; az újabbakat még nem néztük át. ')
            + 'A forrás nem mondja meg, miért, és az oldal nem találgat: az érintett idő itt marad, megjelölve.')
...
def floor_bill_links(inp: dict, bills: list[str], have: set[int], depth: int) -> str:
    ...
    for b in bills:
        n, _ = an.bill_key(b)
        out.append(f'<a href="{"../" * depth}{cycle_dir(inp["cycle"])}iromany/{n}.html">{esc(b)}</a>'
                   if n in have else f'<span>{esc(b)}</span>')
    return " · ".join(out)
...
ANNOUNCE_DEFS: dict[str, str] = {
    ...
    "önálló indítvány minősített többséget igénylő része elfogadva": "A Ház elfogadta az indítvány "
        "minősített többséget igénylő részét. A minősített többség a szavazási mód szerint a jelen "
        "lévő vagy az összes képviselő kétharmada; az ilyen részekről a Ház a többitől külön határoz.",
    ...
    "összegző módosító javaslat minősített többséget igénylő része elfogadva": "A Ház elfogadta az "
        "összegző módosító javaslat minősített többséget igénylő részét. A minősített többség a "
        "szavazási mód szerint a jelen lévő vagy az összes képviselő kétharmada; az ilyen részekről a "
        "Ház a többitől külön határoz.",
...
GATEWAY_DEFS: dict[str, str] = {
    "név szerinti lista": "A név szerinti szavazás névsora: nevenként rögzíti, ki mit szavazott — a jelenlét "
        "megállapításánál azt, ki van jelen. Ilyen lista 1998 előttről egyetlen szavazásról maradt fenn — "
        "minden másról csak összesítés van.",
...
SCHOOL_WORD = re.compile(r"egyetem|f[őo]iskol|universit|akad[ée]mi|tan[ií]t[óo]k[ée]pz|gimn[áa]zium|technikum|iskol|college|school|hochschule|konzervat[óo]ri", re.I)
...
def kind_glossary(kinds, title: str = "Mit jelentenek a jogcímek?") -> str:
    ...
    return ('<details class="how"><summary>' + esc(title) + '</summary><div class="prose">'
            ...
            'szerepei köznyelven.<span class="hover-hint"> Az egeret a jogcímre húzva ugyanez jelenik meg.</span></p><dl class="gloss">'
            + "".join(f'<dt>{esc(k)}</dt><dd>{esc(d)}</dd>' for k, d in gl)
            + '</dl></div></details>')
...
def speakers_panel(inp: dict) -> str:
  ...
  frakció kap rá keretet, és többnyire a kijelölt szónoka mondja el. Aki nem képviselő, akkor szerepel itt, ha a
  jegyzőkönyv sorai azonosítóval kötik hozzá a felszólalását — akit csak névvel említ a lista, az ebből a
  táblából kimarad; náluk a frakció rovat üres. A teljes jogcímenkénti bontás a CSV-ben van. Ami a jegyzőkönyvben hossz nélkül
...
<section class="panel">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0" style="border:0"><table><caption class="vh">Az irományt meg nem nevező napirendi pontok és a rájuk fordított idő</caption><thead><tr><th scope="col" class="num">Idő</th>
...
<section class="panel">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table data-page-size="25" data-counter="dv"><caption class="vh">Az év minden vitája időrendben, a leghosszabbal kezdve</caption><thead><tr><th scope="col" class="num">Idő</th>
...
<section class="panel deep">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table id="oevk" data-page-size="30" data-counter="on"><thead><tr><th scope="col">Választókerület <span class="sub">({def_term("OEVK", GATEWAY_DEFS)})</span></th><th scope="col">Képviselő</th><th scope="col">Frakció</th><th scope="col">Csatornák <span class="sub">({def_term("Atom-csatorna", GATEWAY_DEFS)})</span></th></tr></thead><tbody>{erows or '<tr><td colspan="4">—</td></tr>'}</tbody></table></div>
</section>
<section class="panel">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table id="lista" data-page-size="30" data-counter="ln"><thead><tr><th scope="col">Mandátum</th><th scope="col">Képviselő</th><th scope="col">Frakció</th><th scope="col">Csatornák</th></tr></thead><tbody>{lrows or '<tr><td colspan="4">—</td></tr>'}</tbody></table></div>
</section>
""" + page_tail(inp, 1)
...
def build_cycle(out_dir: Path, cycle: int, index_only: bool = False, light: bool = False) -> dict:
    ...
    if not index_only and not light:
        ...
        jvp = build_remuneration_page(inp) if not inp["closed"] else ""
        if jvp:
            jd = out_dir / "javadalmazas"; jd.mkdir(parents=True, exist_ok=True)
            (jd / "index.html").write_text(jvp, encoding="utf-8")
        ...
        if not inp["closed"] and any(m.get("remuneration") for m in inp["mps"].values()):
            (ad / "javadalmazas.csv").write_text(ex.remuneration_csv(inp["mps"], cycle), encoding="utf-8")
...
def build_profile_page(inp: dict, rows: list[dict]) -> str:
    ...
    for name in DEGREE_ORDER + sorted(k for k in deg if k and k not in DEGREE_ORDER):
        ...
        drows.append(f'<tr><th scope="row">{esc(name)}</th><td class="num mono">{hu_num(v)}</td><td class="num mono">{100 * v / deg_known:.0f}%</td></tr>')
    lang = Counter()
    ...
    lrows = "".join(f'<tr><th scope="row">{esc(k)}</th><td class="num mono">{hu_num(v)}</td>'
    ...
    for p in people.values():
        for t in p.get("local_offices") or []:
            if t.get("body_kind"):
                kinds[t["body_kind"]] += 1
    krows = "".join(f'<tr><th scope="row">{esc(k)}</th><td class="num mono">{hu_num(v)}</td></tr>' for k, v in kinds.most_common(6))
...
<section class="grid">
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><tbody>{"".join(drows)}</tbody></table></div>
  </section>
  <section class="panel">{CORNERS}
    <h2><span data-kz-text>Önkormányzati múlt</span><span class="tag">{hu_num(local)} embernél szerepel ilyen tisztség</span></h2>
    <div class="tablewrap" tabindex="0" style="border:0"><table><tbody>{krows}</tbody></table></div>
...
<section class="grid">
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><tbody>{lrows}</tbody></table></div>
  </section>
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th>Ciklus</th><th class="num">Fő</th><th class="num">Végzettség</th><th class="num">Nyelv</th><th class="num">Önkormányzat</th></tr></thead><tbody>{fill}</tbody></table></div>
...
<section class="panel deep cov">{CORNERS}
  ...
  <div class="covwrap" tabindex="0"><svg viewBox="0 0 {width:.0f} {height:.0f}" preserveAspectRatio="none" role="img"
...
<section class="grid">
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th>Ciklus</th><th class="num">Szavazás</th><th class="num">Név szerint</th><th class="num">Titkos</th><th class="num">Csonka nap</th></tr></thead><tbody>{"".join(trs)}</tbody></table></div>
  </section>
  <section class="panel">{CORNERS}
    <h2><span data-kz-text>Amit nem lehet pótolni</span></h2>
    <div class="prose hero-meta">
      <p><b>Név szerinti lista 1998 előttről egyetlen szavazásról maradt fenn.</b> A 34. és a 35. ciklus többi szavazásánál nincs meg, ki hogyan szavazott: a lekérdezés üres névsort kap. Ez nem a letöltés hibája: az Országgyűlés adatbázisa ebből az időszakból — azon az egy szavazáson kívül, amelyet a lenti táblázat mutat — nem őrzi, ki hogyan szavazott. Ami megvan: hány igen, hány nem, hány tartózkodás. Az oldal sem mutat többet.</p>
...
def profile_html(latest: dict, stints: list[dict]) -> str:
    ...
    sc = sorted((x for x in (latest.get("schools") or []) if x.get("year") and x.get("institution")),
                key=lambda x: -x["year"])
    if sc:
        # A line labelled "Végzettsége" has to point at a school, and the newest row is not always one: the
        # record files the bar exam (institution: a ministry), specialist exams and courses under schooling
        # too. The pick is a lexical filter, not a judgement — the newest row whose institution NAMES a school
        # in its own words (egyetem, főiskol-, universit-, akadémi-, iskol-…; stems, because the Hungarian
        # plural turns the final a into á and "főiskolák" must match). Bóka János's Brussels degree stays
        # because "Universiteit" matches; where no row carries a school word — a handful of intézet and Kft
        # course rows — the newest row is printed in full, degree and institution together, which is true.
        # Measured before shipping: 2 521 first rows keep, 127 pages improve, 20 fall through.
        pick = next((r for r in sc if SCHOOL_WORD.search(r["institution"])), sc[0])
        what = f'{esc(pick["degree"])} — {esc(pick["institution"])}' if pick.get("degree") else esc(pick["institution"])
        bits.append(f'Végzettsége: {what} ({pick["year"]}).')
    if not bits:
        return ""
...
def life_path_html(inp: dict, latest: dict, stints: list[dict]) -> str:
    ...
    return (f'<section class="panel life">{CORNERS}'
            ...
            f'<div class="lifewrap" role="img" aria-label="{esc(latest["name"])} pályája {lo + 1}–{hi - 1}: '
...
def remuneration_html(inp: dict, latest: dict) -> str:
    ...
    # a bar with one component is a full-width rectangle that conveys nothing — the position strip carries
    # the picture instead, so the bar is drawn only when there are parts to compare
    bar = ("".join(f'<i style="--w:{100 * v / total:.2f}%;--o:{1 - 0.3 * i:.2f}" title="{esc(lbl)}: {hu_num(v)} Ft"></i>'
                   for i, (lbl, v) in enumerate(parts) if v)
           if sum(1 for _l, v in parts if v) > 1 else "")
    paybar_html = f'<div class="paybar">{bar}</div>' if bar else ""
    ...
    # Where this sits in the House, same month only: the reader who asked "miért ennyi?" gets the comparison
    # the record supports instead of a lone figure. The median and the rank are computed over every current
    # member whose datasheet states an amount for the same period — a different month would be a different
    # question, so a mismatched period yields no line at all.
    peers = sorted((int(x["gross"]) for m in inp["mps"].values() for x in (m.get("remuneration") or [])
                    if x.get("gross") and x.get("period") == p.get("period")), reverse=True)
    house_ctx = ""
    if len(peers) >= 20 and p.get("gross"):
        med = peers[len(peers) // 2] if len(peers) % 2 else (peers[len(peers) // 2 - 1] + peers[len(peers) // 2]) // 2
        rank = 1 + sum(1 for x in peers if x > p["gross"])
        house_ctx = (f'<div class="hero-meta" style="margin-top:6px">Ugyanebben a hónapban {hu_num(len(peers))} '
                     f'képviselő adatlapja közöl összeget; a középső érték {hu_num(med)} Ft. Ez az összeg '
                     f'{hu_the(rank)} {hu_num(rank)}. helyen áll a Házban. Miből adódnak a különbségek, azt az adatlap nem '
                     f'közli: csak a bruttó összeg szerepel, tételes bontás nélkül.</div>')
    # The member's own multiplier, from the same anchor the House page finds — one implementation, so the
    # two surfaces cannot disagree. Printed only on a forint-exact match, with the statute's own band named:
    # the band, not the person's role, because several offices share a multiplier and the datasheet does not
    # say which one this is.
    band_note = ""
    abase, _cov, amults = remuneration_anchor(inp)
    if abase and p.get("gross"):
        hit = next(((r, m) for r, m, _c in OGYTV_SCHEDULE
                    if abs(p["gross"] - abase * float(m.replace(",", "."))) <= 1), None)
        if hit and hit[1] == "1,00":
            band_note = ('<div class="hero-meta" style="margin-top:6px">Ez maga az alapösszeg '
                         '(Ogytv. 104. § (1)) — tisztségi szorzó nélkül.</div>')
        elif hit:
            cite = next(c for r, m, c in OGYTV_SCHEDULE if m == hit[1])
            rag = {"1,80": "szorosa", "2,00": "szerese", "2,20": "szorosa",
                   "2,40": "szerese", "2,65": "szöröse", "2,91": "szerese"}.get(hit[1], "szorosa")
            band_note = (f'<div class="hero-meta" style="margin-top:6px">Az összeg az alap {hit[1]}-{rag}. '
                         f'A törvényben ez a sáv: {esc(hit[0])} ({esc(cite)}). Hogy e közül melyik illeti, '
                         f'azt az adatlap nem közli.</div>')
        else:
            band_note = ('<div class="hero-meta" style="margin-top:6px">Az összeg nem az alap törvényi '
                         'szorzója — tört hónap, levonás vagy más ok, amit az adatlap nem közöl.</div>')
    # a thin position strip: where this member sits between the month's smallest and largest, median ticked
    strip = ""
    if peers and p.get("gross"):
        lo, hi = peers[-1], peers[0]
        span = (hi - lo) or 1
        mepct = 100 * (p["gross"] - lo) / span
        medv = peers[len(peers) // 2] if len(peers) % 2 else (peers[len(peers) // 2 - 1] + peers[len(peers) // 2]) // 2
        mdpct = 100 * (medv - lo) / span
        strip = (f'<div class="ppos" aria-hidden="true"><i class="med" style="left:{mdpct:.1f}%"></i>'
                 f'<i class="me" style="left:{mepct:.1f}%"></i></div>')
    # The boundary the record draws, said out loud on the pages where a reader will wonder: a government
    # office's pay is not part of the MP datasheet, so a prime minister can stand here with the base figure.
    # What the law says about the government office's pay, verified against njt.hu (two researchers + an
    # adjudicator): the prime minister's illetmény is a statutory formula — the Speaker's honorarium — and
    # the MP base is due ALONGSIDE it (Ogytv. 106. § (2)); a minister's or state secretary's amount is no
    # statutory constant at all — the PM sets it yearly and the Kormányiroda publishes it. Both sentences
    # state the law's own content; neither prints a forint figure the statutes do not.
    gov = [o.get("office") for o in (latest.get("offices") or [])
           if not o.get("to") and any(w in (o.get("office") or "").lower() for w in ("miniszterelnök", "miniszter", "államtitkár"))]
    gov_note = ""
    if gov:
        if any((o or "").strip().lower() == "miniszterelnök" for o in gov):
            gov_note = ('<div class="hero-meta" style="margin-top:6px">Miniszterelnökként a törvény szerinti '
                        'illetménye az Országgyűlés elnökének tiszteletdíjával azonos összeg (Ksztv. 25/A–25/B. §), '
                        'és a képviselői alap-tiszteletdíj e mellett jár (Ogytv. 106. § (2)). Forintösszeget a '
                        'törvény nem rögzít; ezen a lapon csak az országgyűlési rész szerepel.</div>')
        else:
            gov_note = (f'<div class="hero-meta" style="margin-top:6px">Kormányzati tisztségének '
                        f'({esc(gov[0])}) illetményét a törvény nem rögzíti: évente a miniszterelnök állapítja '
                        f'meg (Ksztv. 52. §), és a Miniszterelnöki Kormányiroda teszi közzé (Kit. 186. § (2)). '
                        f'Ezen a lapon csak az országgyűlési javadalmazás szerepel.</div>')
    return (f'<section class="panel">{CORNERS}'
            f'<h2><span data-kz-text>Javadalmazás</span><span class="tag">{esc(p.get("period") or "")} · bruttó · <a href="../{cycle_dir(inp["cycle"])}javadalmazas/index.html">az egész Ház</a></span></h2>'
            f'{paybar_html}'
            f'<div class="tablewrap" tabindex="0" style="border:0"><table><tbody>{lines}</tbody></table></div>'
            + strip + house_ctx + band_note + gov_note +
...
# HU_MONTH_NUM, not HU_MONTHS: the module already has an HU_MONTHS list that hu_date() indexes into, and the
# first version of this dict shadowed it — every date on the site would have crashed the build. The same
# shadowing class as svg_series's `name`, caught the same way: by the build failing loudly.
HU_MONTH_NUM = {"január": "01", "február": "02", "március": "03", "április": "04", "május": "05",
                "június": "06", "július": "07", "augusztus": "08", "szeptember": "09", "október": "10",
                "november": "11", "december": "12"}


def remuneration_anchor(inp: dict):
    """The anchor is FOUND, not assumed: the distinct gross value whose statutory multiples (OGYTV_SCHEDULE)
    land on the most rows to the forint. "Smallest full-month amount" was tried first and failed on its first
    contact with the data — one member draws 322 800 on a full-month mandate, the statutory base of nothing.
    One implementation, cached on inp, because the MP widget and the House page must never disagree."""
    if "_rem_anchor" in inp:
        return inp["_rem_anchor"]
    gs = [x["gross"] for m in inp["mps"].values() for x in m.get("remuneration") or [] if x.get("gross")]
    mults = [float(m.replace(",", ".")) for _r, m, _c in OGYTV_SCHEDULE]
    def cover(b):
        return sum(1 for g in gs if any(abs(g - b * mu) <= 1 for mu in mults))
    base = max(sorted(set(gs)), key=cover, default=0)
    covered = cover(base) if base else 0
    # a sample too small to distinguish an anchor from itself gets no anchor: one row covers itself at 1,00x
    # and the QA round watched a closed cycle declare a member's 2,20x amount "the base" that way
    if len(gs) < 20 or covered < len(gs) * 0.6:
        base = 0
    inp["_rem_anchor"] = (base, covered, mults)
    return inp["_rem_anchor"]


def build_remuneration_page(inp: dict) -> str:
    """javadalmazas/index.html — the whole House's stated month, in one sortable place.

    Everything on it is the datasheet's own gross figure or arithmetic over the set of them: median, rank,
    a bar per row. What it will not do is explain any one member's amount — the source publishes no
    breakdown — so the page shows the adjacent facts instead (current offices, a part-month mark where the
    mandate's own dates fall inside the month) and prints the statute's schedule separately, cited, as what
    the law says rather than as a per-person reconciliation."""
    cycle = inp["cycle"]
    rows = []
    for azon, m in inp["mps"].items():
        r = next((x for x in m.get("remuneration") or [] if x.get("gross")), None)
        if r:
            rows.append((azon, m, r))
    if not rows:
        return ""
    period = rows[0][2].get("period") or ""
    rows.sort(key=lambda t: -t[2]["gross"])
    gs = [t[2]["gross"] for t in rows]
    med = gs[len(gs) // 2] if len(gs) % 2 else (gs[len(gs) // 2 - 1] + gs[len(gs) // 2]) // 2
    mx = gs[0]
    # the period month as ISO, for the part-month mark: "2026. július" -> "2026-07"
    pm = ""
    parts = period.replace(".", "").split()
    if len(parts) == 2 and parts[1] in HU_MONTH_NUM:
        pm = f"{parts[0]}-{HU_MONTH_NUM[parts[1]]}"
    base, covered, mults = remuneration_anchor(inp)
    fac_order = []
    trs = []
    for azon, m, r in rows:
        fac = m.get("faction") or ""
        if fac and fac not in fac_order:
            fac_order.append(fac)
        offices = ", ".join(o.get("office") for o in (m.get("offices") or []) if not o.get("to") and o.get("office"))
        marks = []
        partial = False
        # no suffix on a date (the house rule for numerals covers "15.-ig" too): the sentence is
        # restructured so the date stands bare
        if pm and (m.get("mandate_to") or "").startswith(pm):
            marks.append(f'tört hónap — a mandátum vége: {esc(hu_date(m["mandate_to"]))}'); partial = True
        if pm and (m.get("mandate_from") or "").startswith(pm):
            marks.append(f'tört hónap lehet — a mandátum kezdete: {esc(hu_date(m["mandate_from"]))}'); partial = True
        sub = f'<span class="sub">{" · ".join(marks)}</span>' if marks else ""
        hit = next((mu for mu in mults if base and abs(r["gross"] - base * mu) <= 1), None)
        ratio = f'{hu_dec(hit, 2)}×' if hit and not partial else "—"
        trs.append(
            f'<tr data-f="{esc(fac)}"><td><a href="../kepviselo/{esc(azon)}.html">{esc(m["name"])}</a>{sub}</td>'
            f'<td>{esc(fac or "—")}</td><td>{esc(offices or "—")}</td>'
            f'<td class="num mono">{hu_num(r["gross"])} Ft</td>'
    ...
    return page_head(f'Javadalmazás · {cycle}. ciklus · karzat',
                     f'A {cycle}. ciklus képviselőinek bruttó országgyűlési javadalmazása ({period}), az adatlapok szerint: medián, tisztségek, letölthető CSV.',
                     1 + inp["base_depth"]) + topbar(inp, [("javadalmazás", None)], 1) + f"""
<div class="hero-h"><h1>Javadalmazás</h1><small class="label" data-kz-text>{cycle}. ciklus · {esc(period)} · bruttó · {hu_num(len(rows))} képviselő</small></div>
<p class="lede">Mennyit fizet az Országgyűlés a képviselőknek: az adatlapok bruttó összege egyetlen hónapra —
a forrás csak a lekérdezés hónapját közli, előzményt nem. A kormányzati tisztségek (miniszterelnök, miniszter,
államtitkár) illetménye nem innen jön, ezen a lapon nem szerepel. A középső érték {hu_num(med)} Ft; a
legkisebb és a legnagyobb közti különbség okait az adatlap nem közli — bontás nincs, csak a végösszeg.</p>
<section class="panel">{CORNERS}
  <h2><span data-kz-text>A Ház, sorban</span><span class="tag">{esc(period)} · <a href="../adatok/javadalmazas.csv">CSV</a></span></h2>
  <div class="filters" role="group" aria-label="Szűrés frakcióra és névre">{fbtn}<input type="search" data-filter-table="jav" placeholder="név" aria-label="Szűrés névre" style="min-width:140px"></div>
  <div class="tablewrap" tabindex="0"><table id="jav" data-page-size="50"><caption class="vh">Képviselőnként a hónap bruttó javadalmazása, a jelenlegi tisztségekkel</caption>
  <thead><tr><th scope="col">Képviselő</th><th scope="col">Frakció</th><th scope="col">Tisztség</th><th scope="col" class="num">Bruttó</th><th scope="col" class="num">Az alap szorzataként</th><th scope="col"><span class="vh">Ábra</span></th></tr></thead>
  <tbody>{"".join(trs)}</tbody></table></div>
  <div class="hero-meta prose" style="margin-top:8px">A tisztség rovat tájékoztat, nem magyaráz: az adatlap nem
  köti össze az összeget a tisztségekkel, és a lap sem teszi. Ahol a mandátum a hónap közben kezdődött vagy
  szűnt meg, a sor jelzi — ilyenkor az összeg tört hónapé lehet.{f' Ami viszont számolható: {hu_the(len(rows))} {hu_num(len(rows))} sorból {hu_num(covered)} forintra pontosan a lenti törvényi szorzók egyike szerint áll {hu_num(base)} Ft-hoz képest.' if base else ''}</div>
</section>
{schedule}
{cite_html(inp, f'{cycle_dir(cycle)}javadalmazas/index.html', f'Javadalmazás — {cycle}. ciklus', f'{cycle}-javadalmazas', csv_href='../adatok/javadalmazas.csv')}
""" + page_tail(inp, 1)


# The statutory schedule, verified by two independent researchers and an adjudicator against njt.hu's
# consolidated text (fetched 2026-08-25; the current wording set by 2026. évi XVII. tv., first applicable to
# July 2026 honoraria — the very month the datasheets state). Multipliers are EXCLUSIVE (105. § (7): the
# highest applies), with one statutory exception: a government member draws the MP honorarium alongside the
# government salary (106. § (2)).
OGYTV_SCHEDULE = [
    ("képviselő (alap)", "1,00", "Ogytv. 104. § (1)"),
    ("bizottsági tag", "1,80", "Ogytv. 104. § (2) a)"),
    ("törvényalkotási bizottság tagja vagy két állandó bizottság tagja", "2,00", "Ogytv. 104. § (2) b)"),
    ("jegyző · bizottsági alelnök · az IPU magyar csoportjának elnöke", "2,20", "Ogytv. 105. § (3)–(4)"),
    ("alelnök · háznagy · bizottsági elnök · frakcióvezető-helyettes", "2,40", "Ogytv. 105. § (1)–(2), 4. § (6), 104. § (5)"),
    ("frakcióvezető", "2,65", "Ogytv. 104. § (4)"),
    ("házelnök", "2,91", "Ogytv. 120. §"),
]


def ogytv_schedule_html() -> str:
    """What the statute says, as a cited table — the law's own multipliers, never a per-person reconciliation."""
    rows = "".join(f'<tr><td>{esc(r)}</td><td class="num mono">{m}×</td><td class="mono">{esc(c)}</td></tr>'
                   for r, m, c in OGYTV_SCHEDULE)
    return f"""<section class="panel">{CORNERS}
  <h2><span data-kz-text>Mit mond a törvény</span><span class="tag">Ogytv., a 2026. július óta hatályos szöveg</span></h2>
  <p class="lede">Az alap a KSH előző évi nemzetgazdasági bruttó átlagkeresetének 1,8-szorosa, március 1-jétől
  a következő év februárjáig rögzítve (Ogytv. 104. § (1)). A tisztségek szorzói nem összeadódnak: több
  megbízatásnál a magasabb jár (105. § (7)). Egy kivétel van: a kormánytag képviselő a kormányzati illetménye
  mellett a képviselői tiszteletdíjat is megkapja — ezért állhat egy miniszterelnök ezen a lapon az
  alapösszeggel (106. § (2)).</p>
  <div class="tablewrap" tabindex="0"><table><caption class="vh">A tisztségek törvényi szorzói</caption>
  <thead><tr><th scope="col">Megbízatás</th><th scope="col" class="num">Szorzó</th><th scope="col">Hivatkozás</th></tr></thead>
  <tbody>{rows}</tbody></table></div>
  <div class="hero-meta prose" style="margin-top:8px">A fenti táblázat „az alap szorzataként” oszlopa nem
  becslés: a lap megkereste azt az összeget, amelynek törvényi szorzói a legtöbb sort forintra pontosan
  kiadják, és a szorzó csak ott jelenik meg, ahol az egyezés forintra pontos. Ahol kötőjel áll, ott az összeg
  nem törvényi szorzó — tört hónap, levonás vagy más ok, amit az adatlap nem közöl, és a lap nem találgat.</div>
</section>"""
...
def schooling_html(latest: dict) -> str:
    ...
    if not sc and not lg:
        # the reader who wonders why there is no schooling deserves the record's own answer: nothing. Saying
        # so beats a silent gap — the iromány pages already state what the datasheet omits — and it keeps the
        # two-panel grid whole instead of leaving the pay panel standing alone.
        return ('<section class="panel">' + CORNERS + '<h2><span data-kz-text>Iskolák, nyelvek</span></h2>'
                '<p class="hero-meta prose">A képviselői adatlap ennél a képviselőnél nem közöl iskolai '
                'végzettséget és nyelvismeretet. Az oldal csak azt mutatja, amit a forrás állít.</p></section>')
    srows = []
    ...
    return (f'<section class="panel">{CORNERS}'
            ...
            + (f'<div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th>Év</th><th>Intézmény</th><th>Végzettség</th></tr></thead>'
...
def build_person_page(inp: dict, azon: str, stints: list[dict]) -> str:
    ...
    # Three different people have been called Tóth István in this House, and three pages called
    # "Tóth István — pályakép" are three pages nobody can tell apart in a search result. The name is not a
    # distinction the record can invent one for, but the cycles they sat in are a fact about each of them.
    cs = sorted({r["cycle"] for r in stints})
    span = f'{cs[0]}.' if len(cs) == 1 else f'{cs[0]}–{cs[-1]}.'
    # Two men called Dr. Szabó János sat in the 34th cycle, one for the EKgP and one for the MDF, and the
    # cycle alone does not tell their pages apart. The faction does, and it is the first thing anyone looking
    # for one of them would know.
    fac = next((r.get("faction") for r in stints if r.get("faction")), "")
    who = f'{name} ({fac})' if fac else name
    return page_head(f'{who} — pályakép, {span} ciklus · karzat', f'{name} az Országgyűlésben: ciklusonként a részvétele és a frakciójával való egyezése, a képviselői adatlap frakció- és mandátumtörténete.', 1) + \
...
<section class="panel deep">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table><thead><tr><th scope="col">Ciklus</th><th scope="col">Frakció</th><th scope="col">Mandátum</th><th scope="col" class="num">Névsor</th><th scope="col" class="num">Leadott</th><th scope="col" class="num">Frakciójával</th><th scope="col" class="num">Ellene</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div>
...
<section class="grid">{schooling_html(latest)}{remuneration_html(inp, latest)}</section>
...
<section class="panel deep">{CORNERS}
  ...
  <div class="tablewrap" tabindex="0"><table><thead><tr><th scope="col">Ciklus</th><th scope="col">Bizottságok</th><th scope="col" class="num">Felszólalás</th></tr></thead><tbody>{"".join(rows) or '<tr><td colspan="3">—</td></tr>'}</tbody></table></div>
...
<section class="grid">
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th>Bizottság</th><th>Tisztség</th><th>Időszak</th></tr></thead><tbody>{com_rows or '<tr><td colspan="3">—</td></tr>'}</tbody></table></div>
  </section>
  <section class="panel">{CORNERS}
...
<section class="grid">
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><tbody>
...
def chamber_today_svg(cur: dict) -> tuple[str, list[tuple[str, int, str]]]:
    ...
    for azon, xy in sorted(plan["coords"].items(), key=lambda kv: (kv[1]["sector"], kv[1]["row"], kv[1]["seat"])):
        ...
        # A link, not a `role="button"` with a script behind it. Every one of the 199 seats was announced to a
        # screen reader as a button and not one of them could be reached by a keyboard — no href, no tabindex —
        # so the front page's centrepiece led nowhere without a mouse, and nowhere at all without JavaScript.
        # Each seat now goes to that person's career page, which exists for all 199 and does not move at the
        # next election the way their per-cycle page does. The inspector still opens on click; it just is no
        # longer the only way through.
        parts.append(f'<a class="seat" href="szemely/{esc(azon)}.html" data-az="{esc(azon)}" data-f="{esc(fac)}" '
                     f'aria-label="{esc(mp.get("name") or azon)} ({esc(fac)}) — pályakép">'
                     ...
                     f'<circle cx="{xy["x"]}" cy="{xy["y"]}" r="{hit_r:.2f}" class="hit"/></a>')
    n_sz = 0
    for key, r in sorted(sz_by_seat.items()):
        ...
        if not o:
            continue
        n_sz += 1
        parts.append(f'<a class="seat sz" href="szemely/{esc(r["p_azon"])}.html" data-az="{esc(r["p_azon"])}" data-f="szószóló" aria-label="{esc(r["name"] or "")} ({esc(r["nationality"] or "")} nemzetiségi szószóló)">'
                     ...
                     f'<circle cx="{o["x"]}" cy="{o["y"]}" r="{hit_r:.2f}" class="hit"/></a>')
    n_km = 0
    for key, r in sorted(km_by_seat.items()):
        ...
        if not o:
            continue
        n_km += 1
        parts.append(f'<a class="seat km" href="szemely/{esc(r["p_azon"])}.html" data-az="{esc(r["p_azon"])}" data-f="kormánytag" aria-label="{esc(r["name"] or "")} ({esc(r.get("office") or "kormánytag")}, nem képviselő)">'
...
def build_404() -> str:
    ...
    cur["base_depth"] = 0
    tot = site_totals()
    # CloudFront serves this document at WHATEVER path missed — /ckl43/anything/ included — so relative asset
    # paths resolve wrong everywhere except the root, and the page rendered unstyled exactly where a lost
    # reader met it. Every asset and link on this one page must be root-absolute.
    head = page_head("Nincs ilyen oldal · karzat", "A keresett oldal nincs meg — vissza a karzat nyitólapjára.", 0)
    head = head.replace('href="assets/', 'href="/assets/').replace('src="assets/', 'src="/assets/')
    foot = terminal_html(cur, [f"{len(tot['cycles'])} ciklus · {hu_date(tot['from'])} – {hu_date(tot['to'])} · {hu_num(tot['votes'])} szavazás",
                               "404 — a cím nem található"])
    # the footer's fact line links wherever the day's fact points; on this page every link must survive
    # being served at any depth, so anything still relative gets the root prefix
...
{foot}<script src="/assets/karzat.js"></script>
</body>
</html>
"""
...
def build_landing() -> str:
    ...
    # the legend is information first (names, counts, colours) and a filter only when a script runs: it
    # ships as inert spans and karzat.js upgrades each into the toggle button — a static <button> would be
    # visible, focusable and dead exactly for the readers who have no script
    legend_html = "".join(f'<span class="f" data-hf="{esc(f)}" data-f="{esc(f)}">'
                          f'<i style="{"border:2px solid " + col + ";background:transparent" if f == "szószóló" else "background:" + col}"></i>{esc("nemzetiségi szószóló" if f == "szószóló" else f)} <span class="mono">{n}</span></span>' for f, n, col in legend)
    ...
    n_paid = sum(1 for m in cur["mps"].values() if any(x.get("gross") for x in (m.get("remuneration") or [])))
    # the same </ guard the sibling JSON embeds carry: the payload holds free-text strings,
    # and an unguarded </script> inside one would end the block mid-JSON
    hall_json = (json.dumps(hall_data(cur), ensure_ascii=False, separators=(",", ":"))
                 .replace("</", "<\\/"))
    lines = [
...
<section class="masthead">{CORNERS}
  ...
  <figure class="chamber-today">{chamber}
    ...
    <script type="application/json" id="hall-data" data-mp-base="{cdir}kepviselo/">{hall_json}</script>
  </figure>
</section>
...
<section class="grid land">
  <section class="panel">{CORNERS}
    <h2><span data-kz-text>Ciklusok {hu_date(tot['from'])[:4]} óta</span><span class="tag">mandátumok az {def_term("alakuló ülés", GATEWAY_DEFS)} napján · a képviselői adatlapokból</span></h2>
  ...
  <section class="panel">{CORNERS}
    ...
    <div class="tablewrap" tabindex="0" style="border:0"><table><thead><tr><th scope="col">Idő</th><th scope="col">Tárgy</th><th scope="col">Eredmény</th></tr></thead><tbody>{rows_html or '<tr><td colspan="3">—</td></tr>'}</tbody></table></div>
...
<section class="doors">
  ...
  {f'<a class="panel door" href="{cdir}javadalmazas/index.html">{CORNERS}<h2><span data-kz-text>Javadalmazás</span></h2><p>Mennyit keresnek a képviselők: {hu_num(n_paid)} képviselő egy hónapra kapott bruttó összege egy táblában, mellette a törvényi szorzók. Az adatlap mindig csak a legutóbbi hónapot közli, ezért a tábla a mostani ciklusé.</p><span class="go mono">{cdir}javadalmazas/ →</span></a>' if n_paid else ""}
...
def build_all(out_dir: Path, index_only: bool = False, cycles: list[int] | None = None,
    ...
    if full:
        ...
        (out_dir / "robots.txt").write_text(ROBOTS.format(site=SITE_URL or ""), encoding="utf-8")
    ...
    if not index_only and full:
        ...
        for azon, r in km_people.items():
            ...
            people_n += 1
        prune(pd, pw.written)
    # Last, because it reads the tree rather than being told about it. It used to be written with robots.txt,
    # before the cross-cycle pages and the career pages existed — so on a clean build it listed the ten cycles
    # and nothing else: no keresés, no módszer, no arcél, and none of the 1,619 people, who are the half its
    # own docstring calls the point. Every previous build was incremental, and found last time's szemely/ still
    # on disk, which is why this only appeared the first time I emptied site/ before building.
    if full:
        (out_dir / "sitemap.xml").write_text(sitemap(out_dir), encoding="utf-8")
